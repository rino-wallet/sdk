...
export class MoneroWallet {
  ...
  public static init = async (
    config: WalletConfig,
    open?: boolean,
  ): Promise<MoneroWallet> => {
    const wallet = open
      ? await monerojs.openWalletFull(config)
      : await monerojs.createWalletFull(config);

    return new MoneroWallet(wallet);
  };
   ...
   */
  public getWalletJSON = async (): Promise<IWalletData> =>
    parseWalletData(this.wallet);
   ...
   * @returns Promise represents the multisig string
   */
  public makeMultisig = async (
    peerMultisigHexes: string[],
    numberOfParticipants: number,
    walletPassword: string,
  ): Promise<Multisig> => {
    const baseXinfo = await this.wallet.makeMultisig(
      peerMultisigHexes,
   ...
   * @returns Promise
   */
  public exchangeMultisigKeys = async (
    multisigHexes: string[],
    walletPassword: string,
  ): Promise<ExchangeMultisigKeysResult> => {
    const result = await this.wallet.exchangeMultisigKeys(
      multisigHexes,
      walletPassword,
    );
    return result;
   ...
   * (i.e. that the wallet can spend one time).
   */
  ...
  public importOutputs = async (outputsHex: string): Promise<number> =>
    this.wallet.importOutputs(outputsHex);

  /**
   * Export this wallet's multisig info as hex for other participants.
   */
  public getMultisigHex = async (): Promise<string> =>
    this.wallet.exportMultisigHex();

  public getSubaddresses = async (
    index: number,
  ...
  public importMultisigHex = async (multisigHexes: string[]): Promise<number> =>
    this.wallet.importMultisigHex(multisigHexes);

  /**
   * Reconstruct and validate the transaction, returns txHex
   */
  public reconstructAndValidateTransaction = async (
    txHex: string,
   ...
   */
  public loadMultisigTx = async (
    txHex: string,
  ): Promise<{ state: TransactionConfig }> => {
    try {
